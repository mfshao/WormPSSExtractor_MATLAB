    ...
    for i = 1:size(branchpoint,1)
        ...
        if ~isempty(branchNeighbors)
            ...
            for j = 1: size(branchNeighbors, 1)
                ...
                if  isempty(find(ismember([branchNeighbors(j,:),branchpoint(i,:)] , edges(:, end-3: end), 'rows'), 1))...
                     ...
                     %list
                     y = find(ismember(branchNeighbors(j,:), endpoint,'rows'), 1);
                     x = find(ismember(branchNeighbors(j,:), branchpoint,'rows'), 1);
                     ...
                     if ~isempty(x) || ~isempty(y)
                        ...
                        index = index + 1;
                         ...
                         while endLoop == 0
                             ...
                             if minDist == 1
                                minIndex = distFromCurrent == min(distFromCurrent(:));
                             ...
                             x = find(ismember(pixelList(end,:), branchpoint,'rows'), 1);
                             y = find(ismember(pixelList(end,:), endpoint,'rows'), 1);
                            if ~isempty(x) || ~isempty(y)
